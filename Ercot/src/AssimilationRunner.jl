...
function observation_from_latest(db::DuckDB.DB, config::AssimilationStateConfig)
    ...
    observed[:load] = (Float64(coalesce(get(fact, :system_load_forecast_mw, missing), 0.0)), 25_000.0)
    observed[:wind] = (Float64(coalesce(get(fact, :wind_system_mw, missing), 0.0)), 10_000.0)
    observed[:solar] = (Float64(coalesce(get(fact, :solar_system_mw, missing), 0.0)), 5_000.0)
    observed[:thermal_outage] = (Float64(coalesce(get(fact, :outage_resource_south_mw, missing), 0.0)), 10_000.0)
    observed[:lambda] = (Float64(coalesce(get(fact, :system_lambda, missing), 0.0)), 1.0)
    observed[:prc] = (Float64(coalesce(get(fact, :prc, missing), 0.0)), 4.0)
    observed[:mcpc_ecrs] = (Float64(coalesce(get(fact, :mcpc_ecrs, missing), 0.0)), 4.0)
    observed[:mcpc_nspin] = (Float64(coalesce(get(fact, :mcpc_nspin, missing), 0.0)), 4.0)
