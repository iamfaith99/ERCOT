...
class BatterySimulator:
    ...
    def can_sustain_as_duration(
        self,
        service: str,
        mw: float,
        duration_hours: float,
        projected_energy_change_mwh: float = 0.0,
    ) -> bool:
        """
        Check if battery can sustain AS commitment for required duration.
        
        This is critical for RTC+B where SCED must verify SOC constraints
        before awarding AS. For example, Regulation requires 30 minutes
        of sustained capability.
        ...
        mw : float
            AS commitment amount (MW)
        duration_hours : float
            Required duration (hours, e.g., 0.5 for 30 minutes)
        projected_energy_change_mwh : float
            Projected energy change from energy market dispatch (MWh)
            Positive = discharge, Negative = charge
        
        Returns
        -------
        bool
            True if battery can sustain AS for required duration
        """
        if mw <= 0:
            return False
        ...
        # Calculate current available energy considering projected changes
        current_energy_mwh = self.state.energy_mwh + projected_energy_change_mwh
        current_soc = current_energy_mwh / self.specs.capacity_mwh
        
        # Clamp to valid SOC range
        current_soc = np.clip(current_soc, self.specs.min_soc, self.specs.max_soc)
        current_energy_mwh = current_soc * self.specs.capacity_mwh
        
        if service in ["reg_up", "rrs", "ecrs"]:
            # Need discharge capability for duration
            # Energy required = MW * duration / efficiency (accounting for losses)
            energy_required_mwh = mw * duration_hours / self.specs.discharge_efficiency
            
            # Minimum SOC needed to sustain this discharge
            min_energy_needed = self.specs.min_soc * self.specs.capacity_mwh + energy_required_mwh
            min_soc_needed = min_energy_needed / self.specs.capacity_mwh
            
            # Check if current SOC (after projected change) is sufficient
            available_energy = current_energy_mwh - (self.specs.min_soc * self.specs.capacity_mwh)
            return available_energy >= energy_required_mwh and current_soc >= min_soc_needed
            
        elif service == "reg_down":
            # Need charge capability for duration
            # Energy capacity = MW * duration * efficiency (accounting for losses)
            energy_capacity_mwh = mw * duration_hours * self.specs.charge_efficiency
            
            # Maximum SOC allowed (must leave room for charging)
            max_energy_allowed = self.specs.max_soc * self.specs.capacity_mwh - energy_capacity_mwh
            max_soc_allowed = max_energy_allowed / self.specs.capacity_mwh
            
            # Check if current SOC (after projected change) leaves room for charging
            available_capacity = (self.specs.max_soc * self.specs.capacity_mwh) - current_energy_mwh
            return available_capacity >= energy_capacity_mwh and current_soc <= max_soc_allowed
